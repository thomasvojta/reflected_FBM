...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
!   time-discretee reflected fractional Brownian motion
!   on 2d disk
!
!   fbm_sector1 :  29 Jun 2019      sector instead of full disk, hard wall only 
!   fbm_sector2 :  29 Jun 2019      use MPI_REDUCE instead of MPI_SEND
!   fbm_sector3 :  16 Aug 2019      implement soft wall for sector 
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Preprocessor directives
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!#define PARALLEL
#define VERSION 'fbm_sector3'
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
      ...
      real(r8b),parameter         :: RMAX = 500000.D0                    ! radius of sector
      real(r8b),parameter         :: X0= 0.01D0, Y0=0.D0                  ! starting point
      real(r8b),parameter         :: PHIMAX=Pi/24                   ! angular boundaries (-PHIMAX to PHIMAX)
      
      real(r8b), parameter        :: STEPSIG=1.D0                 ! sigma of individual step
      character(3)                :: STEPDIS='GAU'                 ! Gaussian = GAU, binary = BIN, box = BOX                   

      character(4)                :: WALL='SOFT'                  ! SOFT or HARD     
      ...
      integer(i4b), parameter     :: NBIN =  100000                   ! number of bins for radial distribution
      real(r8b),parameter         :: NARRFAC = 10.D0                 ! narrow distribution between PHIMAX/NARRFAC and PHIMIN/NARRFAC
      logical,parameter           :: WRITEPHIDISTRIB = .TRUE.        ! write angular distribution    
      integer(i4b), parameter     :: NPHIBIN =  400               ! number of bins for angular distribution
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
      ...
      real(r8b), parameter        :: PHIMIN=-PHIMAX
...
! Time loop !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        if ( WALL.eq.'HARD') then                                                ! hard reflecting boundaries
           do it=1, NT
              ...
              rad2= xx(it)**2 + yy(it)**2
              phi= atan2( yy(it), xx(it) )
              if ( (rad2.gt.RMAX2).or.(phi.lt.PHIMIN).or.(phi.gt.PHIMAX) ) then
                 xx(it)=xx(it-1)
                 yy(it)=yy(it-1)
              end if
           end do  
        ...
        if ( WALL.eq.'SOFT') then
           do it=1, NT
              ...
              phi= atan2( yy(it-1), xx(it-1) )
              forcer = F0*exp(LAM*(rad-RMAX))                            ! outer arc
              forcel = F0*exp(-LAM*rad*sin(phi-PHIMIN))                  ! lower radius 
              forceu = F0*exp(-LAM*rad*sin(PHIMAX-phi))                   ! upper radius 
              forcet = F0*exp(-LAM*xx(it-1))
              xx(it)=xx(it-1) +xix(it) -forcer*xx(it-1)/rad - forcel*sin(PHIMIN) + forceu*sin(PHIMAX) +forcet
              yy(it)=yy(it-1) +xiy(it) -forcer*yy(it-1)/rad + forcel*cos(PHIMIN) - forceu*cos(PHIMAX)
           end do  
        endif    
           ...
           if (WRITEDISTRIB) then
              do it = NTSTART,NTEND 
                 ...
                 if ( (ibin.ge.0) .and. (ibin.le.NBIN)) then
                   ...
                   if( (phi.gt.PHIMIN/NARRFAC).and.(phi.lt.PHIMAX/NARRFAC) ) then
...
! Now collect and analyze data !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      if (WRITEDISTRIB) then
        ...
        write(2,*) 'NCONF=',totconf
        write (2,*)'=================================='
        write(2,*) ' ibin  r=(RMAX*ibin)/NBIN  r/RMAX  2*pi*r*P(r)  P(r)  P(r)*RMAX**2   Pnarr(r)  Pnarr(r)*RMAX**2 '
        do ibin=0,NBIN 
          rad= (RMAX*ibin)/NBIN
          PP= (sumdis(ibin)*NBIN)/(RMAX*totconf*(NTEND-NTSTART+1))
          PPnarr = (sumdisnarr(ibin)*NBIN)/(RMAX*totconf*(NTEND-NTSTART+1))
          Write(2,'(1X,I7,8(2X,E13.6))') ibin, rad, rad/RMAX, PP, PP/((PHIMAX-PHIMIN)*rad), PP*RMAX2/((PHIMAX-PHIMIN)*rad),&
     &                                   (PPnarr*NARRFAC)/((PHIMAX-PHIMIN)*rad), (PPnarr*NARRFAC*RMAX2)/((PHIMAX-PHIMIN)*rad)       
        enddo 
        close(2) 
      endif
