...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
      ...
      integer(i4b), parameter     :: NCONF=20000                    ! number of walkers
      real(r8b), parameter        :: GAMMA = 0.4D0              ! FBM correlation exponent 
      
      real(r8b),parameter         :: RMAX = 1.D4                 ! radius
      real(r8b),parameter         :: X0= 0.D0, Y0=0.D0, Z0=0.D0         ! starting point
...
! Time loop !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        if ( WALL.eq.'HARD') then                                                ! hard reflecting boundaries
           do it=1, NT
              ...
              if ( (xx(it)**2 + yy(it)**2 + zz(it)**2) .gt. RMAX2 ) then
                 xx(it)=xx(it-1)
                 yy(it)=yy(it-1)
                 zz(it)=zz(it-1)
              end if
...
! Now collect and analyze data !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      if (WRITEDISTRIB) then
        ...
        do ibin=1,NBIN 
          ...
          volume=(4.D0/3.D0)*Pi*volume
          PPvol= sumdis(ibin)/(volume*totconf*(NTEND-NTSTART+1))
          PPvolerr= sqrt(sumdis(ibin))/(volume*totconf*(NTEND-NTSTART+1))
          Write(2,'(1X,I7,9(2X,E13.6))') ibin, rad, rad/RMAX, PP, PPvol, PPvol*RMAX2, RMAX-rad,PPvolerr
        enddo 
        close(2) 
      endif
